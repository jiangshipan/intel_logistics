        ...
        <el-table :data="customer_list" style="width: 100%" border size="medium">
            ...
            <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                    <el-button class="delete" type="danger" @click="doOption('此操作将把该客户从系统中删除, 是否继续?', scope.row.username, 0)">删除</el-button>
                    <el-button class="be_promoted" type="primary" @click="doOption('此操作将把该客户变为快递员身份, 是否继续?', scope.row.username, status)">成为快递员</el-button>
                </template>
            </el-table-column>
        </el-table>
    </div>
...
export default {
    ...
    data() {
        return {
           ...
           user_incr_url: base_url + 'user/incr'
        }
    },
    mounted() {
    ...
    methods: {
      get_all_customers() {
        axios.get(this.get_customer_url + '?identity_type=2')
        .then(response => {
      ...
      user_increase(username) {
          axios.get(this.user_incr_url + '?username=' + username)
          .then(response => {
              var res = response.data;
              if (res.code == 0) {
                  this.get_all_customers();
